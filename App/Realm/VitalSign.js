...
export class VitalSign {
  store(data) {
    Storage.write(
      realm => {
        const lastRecord = realm
          .objects('VitalSign')
          .sorted('registerAt', true)[0];
        this.compare(data, lastRecord)
          ? (lastRecord.repeat += 1)
          : realm.create('VitalSign', data);
      },
      error => {
  ...
  compare(newRecord, lastRecord) {
    ...
    return (
      _.isEqual(_.pick(newRecord, keys), _.pick(lastRecord, keys)) &&
      Math.round(newRecord.temperature * 10) ===
        Math.round(lastRecord.temperature * 10) &&
      Math.abs(
        newRecord.registerAt - lastRecord.registerAt - lastRecord.repeat
      ) <= 1
    );
  }
