    ...
    Storage.write(
      realm => {
        const lastRecord = realm
          .objects('Alarm')
          .filtered(
            `type = "${data.type}" AND registerAt = ${data.registerAt}`
          );
        this.isEmpty(lastRecord)
          ? realm.create('Alarm', data)
          : ((lastRecord[0].repeat += 1), (lastRecord[0].sync = false));
      },
      error => {
      ...
      realm => {
        data.forEach(item => {
          const lastRecord = realm
            .objects('Alarm')
            .filtered(
              `type = "${item.type}" AND registerAt = ${item.registerAt}`
            );
