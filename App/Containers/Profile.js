...
class Profile extends Component {
  ...
  render() {
    ...
    const babyImage = baby.image
      ? { uri: baby.image.uri }
      : require('../Images/Profile/3-3.jpg');

    const data = [
      {
        title: 'Height',
        icon: 'show-chart',
        value: `${DailyStatHelper.getLastStatFor(
          this.props.stats,
          'height',
          '-'
        )} cm`,
        rightSubtitle: DailyStatHelper.getLastDateFor(
          this.props.stats,
          'height'
        ),
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.topContainer}>
          ...
          <View style={styles.topWrapper}>
            ...
            <View style={styles.babyContainer}>
              ...
              />
              <Text style={styles.title}>{baby.name ? baby.name : 'Baby'}</Text>
        ...
        <View style={styles.bottomContainer}>
          <View style={styles.list}>
            <Text style={styles.listTitle}>baby Information</Text>
            <ListItem
              title={baby.name || 'Baby'}
              leftAvatar={{ source: require('../Images/Profile/3-3.jpg') }}
              onPress={() =>
                this.props.navigation.navigate('EditBabyInformation')
              }
              chevron
            />
          </View>
        ...
        <View style={styles.bottomContainer}>
          <View style={styles.list}>
            <Text style={styles.listTitle}>Daily Information</Text>
            {data.map((item, i) => (
              <ListItem
                key={i}
                title={item.title}
        ...
        <View style={styles.bottomContainer}>
          <View style={styles.list}>
            <Text style={styles.listTitle}>Parent Information</Text>
            {parents.map((item, i) => (
              <ListItem
                scaleProps={{
                  friction: 90,
...
Profile.navigationOptions = {
  ...
  tabBarIcon: ({ tintColor }) => (
    <Icon size={20} name="account-circle" color={tintColor} />
  ),
};
...
const mapDispatchToProps = dispatch => {
  return {
    setBabyImage: (image, previousImage, failure) =>
      dispatch(BabyAction.setImage({ image, previousImage, failure })),
