...
class Dashboard extends Component {
  ...
  status = {
    ...
    disconnected: {
      title: 'Disconnected',
      subtitle: this.props.disconnectError
        ? this.props.disconnectError
        : "you're disconnected from ninix",
      type: 'alert',
      button: (
        <Button
          title="Reconnect"
          ...
          onPress={() =>
            this.props.bluetoothState === BluetoothState.poweredOn
              ? this.props.connect(this.props.device)
              : alert('Please turn on bluetooth first')
          }
          containerStyle={styles.outer}
          buttonStyle={[
            styles.buttonStyle,
    ...
    return [
      {
        ...
        value: isConnected ? (
          <Icon name={DataDisplay.battery(streams)} type="material-community" />
        ) : (
          '-'
        ),
      },
