...
export function *setImage (api, action) {
  ...
  const { auth } = yield select()
  const { channel, uploadImage } = yield call(imageProgressChannel, api, auth.accessToken, action.payload.image)
...
export function imageProgressChannel(api, token, payload) {
  ...
  const channel = eventChannel(emitter => {
    uploadImage = api.storeBabyImage(
      token,
      formData,
...
export function *imageProgress (channel) {
  while (true) {
    try {
      ...
      yield put(BabyAction.imageProgressUpdate(Math.round(event.loaded / event.total * 100)))
    }
    catch (error) {
