...
export function* didAddNewRecord(api, action) {
  ...
  const data = Object.keys(unSyncedData)
    .map(index => unSyncedData[index])
    .map(item => ModelToJson.dailyStat(item));
  ...
  try {
    ...
    yield put(
      DailyStatAction.update(
        result.daily_stats.map(item => JsonToModel.dailyStat(item))
      )
    );
  } catch (error) {
