...
export function* setImage(api, action, type, actionMethod) {
  ...
  const data = FormFactory.create()
    .appendImageToForm(image.uri, image.type, image.fileName)
    .build();
  const { channel, uploadPhoto } = yield call(
    uploadChannel,
    sendPhoto,
    type,
    data
...
export function* uploadProgress(channel, actionMethod) {
  while (true) {
    ...
    yield put(
      actionMethod({ progress: Math.round((event.loaded / event.total) * 100) })
    );
  }
}
